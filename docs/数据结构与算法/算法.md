...
N^3^/6-N^2^/2+N/3。使用 ~f(N) 来表示所有随着 N 的增大除以 f(N) 的结果趋近于 1 的函数。

**2. 增长数量级**

N^3^/6-N^2^/2+N/3的增长数量级为 O(N^3^)。增长数量级将算法与它的具体实现隔离开来，一个算法的增长数量级为 O(N^3^) 与它是否用 Java 实现，是否运行于特定计算机上无关。
...
更有效的方法是先将数组排序，然后使用双指针进行查找，时间复杂度为 O(N^2^)。
...
例如对于暴力的 ThreeSum 算法，近似时间为 ~N^3^/6。进行如下实验：多次运行该算法，每次取的 N 值为前一次的两倍，统计每次执行的时间，并统计本次运行时间与前一次运行时间的比值，得到如下结果：
...
可以看到，T(2N)/T(N) ~ 2^3^，因此可以确定 T(N) ~ aN^3^logN。
...
选择排序需要 ~N^2^/2 次比较和 ~N 次交换，它的运行时间与输入无关，这个特点使得它对一个**已经排序**的数组也需要这么多的比较和交换操作。

![img](https://cs-notes-1256109796.cos.ap-guangzhou.myqcloud.com/bc6be2d0-ed5e-4def-89e5-3ada9afa811a.gif)
...
在一轮循环中，如果没有发生交换，那么说明数组已经是有序的，此时可以**直接退出**。

![img](https://cs-notes-1256109796.cos.ap-guangzhou.myqcloud.com/0f8d178b-52d8-491b-9dfd-41e05a952578.gif)
...
每次都将当前元素插入到左侧已经排序的数组中，使得**插入之后左侧数组依然有序**。
...
![img](https://cs-notes-1256109796.cos.ap-guangzhou.myqcloud.com/35253fa4-f60a-4e3b-aaec-8fc835aabdac.gif)
...
对于大规模的数组，插入排序很慢，因为它**只能交换相邻的元素**，**每次只能将逆序数量减少 1**。希尔排序的出现就是为了解决插入排序的这种局限性，它**通过交换不相邻的元素，每次可以将逆序数量减少大于 1**。
...
![img](https://cs-notes-1256109796.cos.ap-guangzhou.myqcloud.com/c4859290-e27d-4f12-becf-e2a5c1f3a275.gif)
...
![img](https://cs-notes-1256109796.cos.ap-guangzhou.myqcloud.com/99d5e84e-fc2a-49a3-8259-8de274617756.gif)
...
![img](https://cs-notes-1256109796.cos.ap-guangzhou.myqcloud.com/4bf5e3fb-a285-4138-b3b6-780956eb1df1.gif)
...
![img](https://cs-notes-1256109796.cos.ap-guangzhou.myqcloud.com/d156bcda-ac8d-4324-95e0-0c8df41567c9.gif)
